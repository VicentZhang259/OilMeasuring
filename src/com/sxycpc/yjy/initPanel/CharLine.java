...
public class CharLine {
	...
	public JFreeChart createChart(XYSeriesCollection dataSet) {
		...
		// standardChartTheme.setLowerBound(0.0); //数据轴上的显示最小值
		// standardChartTheme.setUpperBound(1.0); //数据轴上的显示最大值
		// 通过ChartFactory创建JFreeChart
		JFreeChart chart = ChartFactory.createXYLineChart("渗透率统计", // 图表标题
				"SW", // 横轴标题
				"渗透率（%）", // 纵轴标题
				// createDataSet(),//数据集合
				dataSet, PlotOrientation.VERTICAL, // 图表方向
				true, // 是否显示图例标识
				false, // 是否显示tooltips
				false);// 是否支持超链接		
		XYPlot xyplot = chart.getXYPlot();
		xyplot.getDomainAxis().setUpperMargin(0.05);
		xyplot.getDomainAxis().setLowerMargin(0.05);
		xyplot.getRangeAxis().setLowerMargin(0.05);
